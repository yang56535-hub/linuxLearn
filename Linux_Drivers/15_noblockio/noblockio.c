...
static void key_timer_function(struct timer_list *arg)
{
    ...
    if (0 == current_val && last_val){
        atomic_set(&keydev.status, KEY_PRESS); /* 按下 */
        wake_up_interruptible(&keydev.r_wait); /* 唤醒 */
    }
    else if (1 == current_val && !last_val) {
        atomic_set(&keydev.status, KEY_RELEASE); /* 松开 */
        wake_up_interruptible(&keydev.r_wait); /* 唤醒 */
    }
    else
        atomic_set(&keydev.status, KEY_KEEP); /* 状态保持 */
...
static ssize_t key_read(struct file *filp, char __user *buf, size_t cnt, loff_t *offt)
{
    ...
    if (filp->f_flags & O_NONBLOCK) { /* 非阻塞方式访问 */
        ...
        add_wait_queue(&keydev.r_wait, &wait);           /* 将队列项添加到等待队列头 */
        ...
        remove_wait_queue(&keydev.r_wait, &wait);   /* 将对应的队列项从等待队列头删除 */
    }
    /* 将按键状态信息发送给应用程序 */
    ...
    return ret;
out:
    __set_current_state(TASK_RUNNING);          /* 将当前人物设置为运行状态 */
	remove_wait_queue(&keydev.r_wait, &wait);   /* 将对应的队列项从等待队列头删除 */
    return -EINVAL;
}
...
static unsigned int key_poll(struct file *filp, struct poll_table_struct *wait)
{
    ...
    if(KEY_KEEP != atomic_read(&keydev.status))/* 按键按下或松开动作发生 */
